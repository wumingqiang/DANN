...
def test(feature_extractor, class_classifier, domain_classifier, source_dataloader, target_dataloader):
    ...
    source_accu =  100. * float(source_correct) / len(source_dataloader.dataset)
    target_accu =  100. * float(target_correct) / len(target_dataloader.dataset),
    domain_accu =  100. * float(domain_correct) / (len(source_dataloader.dataset) + len(target_dataloader.dataset))
    result_str = str(f'\nSource Accuracy: ({source_accu})'
                     f'\nTarget Accuracy: ({target_accu })'
                     f'\ndomain Accuracy: ({domain_accu})')
    print(result_str)
    return result_str, source_accu,target_accu,domain_accu